...
export default function TodoDashboard({ user, onLogout }) {
  ...
  // Fetch tasks
  const fetchTasks = () => {
    fetch(`${'https://online-todo-mathu.onrender.com'}/api/tasks`, {
      credentials: 'include',
    })
      .then(res => res.json())
      .then(setTasks);
  ...
  const handleSubmit = async e => {
    ...
    if (collaboratorFields.length) {
      ...
      const res = await fetch(`${'https://online-todo-mathu.onrender.com'}/api/users/by-emails`, {
        method: 'POST',
    ...
    const url = `${'https://online-todo-mathu.onrender.com'}/api/tasks${editingId ? `/${editingId}` : ''}`;
  ...
  const handleDelete = async id => {
    await fetch(`${'https://online-todo-mathu.onrender.com'}/api/tasks/${id}`, {
      method: 'DELETE',
      credentials: 'include',
    });
